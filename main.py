...
for folder in milestones:
    directories = [x[0] for x in os.walk(folder)]
    for directory in directories[1::]:
        print(f'start by {directory}')

        listings = pd.read_csv(f'{directory}/listings.csv',
                               usecols=["scrapedAt", "listingId", "title", "metaPrice.currencyCode", "metaPrice.symbol",
                                        "metaPrice.floatValue", "image", "description", "category", "discount",
                                        "rating",
                                        "reviewsCount", "seller.badge", "seller.avatar", "seller.features",
                                        "seller.tags",
                                        "seller.name", "breadcrumbs", "location", "lat", "lng", "guests",
                                        "pets_allowed",
                                        "description_items", "category_rating", "rules", "details", "highlights",
                                        "neighborhood",
                                        "nearbyCities", "arrangement_details", "amenities", "images", "propertyType",
                                        "url"],
                               sep=';', encoding='utf-8')

        print(f'{listings.size} ready to format...')
        reviews = pd.read_csv(f'{directory}/reviews.csv',
        ...
        prices = pd.read_csv(f'{directory}/prices.csv',
        ...
        def formatting(row):
            ...
            listingId = None if pd.isna(dataRow['listingId']) or dataRow['listingId'] == 'undefined' else dataRow[
                'listingId']
            ...
            meta_price_symbol = None if pd.isna(dataRow['metaPrice.symbol']) or dataRow[
                'metaPrice.symbol'] == 'undefined' else \
            ...
            category = None if pd.isna(dataRow['category']) or dataRow['category'] == 'undefined' else dataRow[
                'category']
            discount = None if pd.isna(dataRow['discount']) or dataRow['discount'] == 'undefined' else dataRow[
                'discount']
            rating = None if pd.isna(dataRow['rating']) or dataRow['rating'] == 'undefined' else dataRow['rating']
            reviews_count = None if pd.isna(dataRow['reviewsCount']) or dataRow['reviewsCount'] == 'undefined' else \
            dataRow[
                'reviewsCount']
            host_badge = None if pd.isna(dataRow['seller.badge']) or dataRow['seller.badge'] == 'undefined' else \
            dataRow[
                'seller.badge']
            host_avatar = None if pd.isna(dataRow['seller.avatar']) or dataRow['seller.avatar'] == 'undefined' else \
            dataRow[
                'seller.avatar']
            ...
            if breadcrumbs is not None: location = breadcrumbs[-1] if pd.isna(dataRow['location']) or dataRow[
            ...
            pets_allowed = None if pd.isna(dataRow['pets_allowed']) or dataRow['pets_allowed'] == 'undefined' else \
            dataRow[
                'pets_allowed']
            category = None if pd.isna(dataRow['category']) or dataRow['category'] == 'undefined' else dataRow[
                'category']
            ...
            if neighborhood is not None and location is None: location = neighborhood.split(',')[0]
            nearbyCities = None
            ...
            propertyType = None if pd.isna(dataRow['propertyType']) or dataRow['propertyType'] == 'undefined' else \
            dataRow[
                'propertyType']
            ...
            fieldnames = ['listingId', 'title', 'price_currency', 'price_symbol', 'price_value', 'meta_price_currency',
                          'meta_price_symbol', 'meta_price_value', 'image',
                          'description', 'category', 'discount', 'rating', 'reviews_count', 'host_badge', 'host_avatar',
                          'host_name', 'host_languages',
                          'host_response_rate',
                          'host_response_time', 'host_reviews_count', 'host_airbnb_supporter', 'host_hasLicense',
                          'host_license_number', 'host_Identity_verified',
                          'host_isExperienced', 'breadcrumbs', 'location', 'description_items', 'beds', 'baths',
                          'bedrooms',
                          'lat',
                          'lng', 'guests', 'pets_allowed', 'category',
                          'AccuracyRating', 'CheckinRating', 'CleanlinessRating', 'CommunicationRating',
                          'LocationRating',
                          'ValueRating', 'rules', 'details',
                          'highlights', 'wifi', 'workspace', 'neighborhood', 'nearbyCities', 'arrangement_details',
                          'amenities',
            ...
            with open('result.csv', mode='a' if file_exists else 'w', newline='') as file:
