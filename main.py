...
for index, row in data.iterrows():
    ...
    meta_price_currency = None if pd.isna(dataRow['metaPrice.currencyCode']) or dataRow['metaPrice.currencyCode'] == 'undefined' else dataRow['metaPrice.currencyCode']
    meta_price_symbol = None if pd.isna(dataRow['metaPrice.symbol']) or dataRow['metaPrice.symbol'] == 'undefined' else dataRow['metaPrice.symbol']
    meta_price_value = None if pd.isna(dataRow['metaPrice.floatValue']) or dataRow['metaPrice.floatValue'] == 'undefined' else dataRow['metaPrice.floatValue']
    image = None if pd.isna(dataRow['image']) or dataRow['image'] == 'undefined' else dataRow['image']
    description = None if pd.isna(dataRow['description']) or dataRow['description'] == 'undefined' else dataRow['description']
    ...
    reviews_count = None if pd.isna(dataRow['reviewsCount']) or dataRow['reviewsCount'] == 'undefined' else dataRow['reviewsCount']
    host_badge = None if pd.isna(dataRow['seller.badge']) or dataRow['seller.badge'] == 'undefined' else dataRow['seller.badge']
    host_avatar = None if pd.isna(dataRow['seller.avatar']) or dataRow['seller.avatar'] == 'undefined' else dataRow['seller.avatar']
    host_name = None if pd.isna(dataRow['seller.name']) or dataRow['seller.name'] == 'undefined' else dataRow['seller.name']
    host_languages = None
    ...
    if not (pd.isna(dataRow['breadcrumbs']) or dataRow['breadcrumbs'] == 'undefined'):
        for b in json.loads(dataRow['breadcrumbs']):
            if b['title'] != 'Airbnb':
                print(b['title'])
                breadcrumbs = []
